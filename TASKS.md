...
- [x] Locate the direct transfer/custody record showing how Todd Sanders obtained the JPMI image/report lineage — **done**: Sanders received the drive copy directly from Brian Della Rocca; mailing-packet photograph (`photo_20260716_120324.jpg`) documents the shipment from Mac Isaac's home address to Sanders.
- [ ] Compare source-image/acquisition hashes to determine whether the JPMI source and the exact-copy media supplied by Della Rocca to CBS were byte-identical (currently assumed from the common source and purpose, not hash-verified).
- [ ] Determine how many copies were made in the preservation-copy period and where each went.
- [ ] Seek an FBI-side explanation, including why the original laptop and external drive seized December 9, 2019 have not been returned to anyone; only the FBI can verify the FBI-side custody history.
- [ ] Locate the complete acquisition worksheet for `HB-IMAGE-2022-04-29.E01`.
- [ ] Reconcile the later 2022 acquisition / 2024 HFS+ last-write report chronology (Sanders' read-write-mount account is recorded but unverified).
...
- [x] The Todd Sanders connection is presented as a documented direct transfer from Della Rocca (mailing-packet photograph), with byte-identity to the CBS media stated as a source-derived assumption pending an independent hash comparison, not as proved fact.
