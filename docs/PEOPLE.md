...
**Date scatter on the Albuquerque visit** (all participant-class):
...
The record preserves the scatter rather than selecting a single day.
...
None of those channels answered, which is the family's stated reason John Paul later went to Costello (Aug 2020).
...
Mac Isaac interpreted DeMeo's “nothing ever happens to people who don't talk” line as a threat. The FBI has not, in the sources used here, published a rebuttal naming those agents.
...
- The JPMI drive arrived via Della Rocca → Todd Sanders ([mailing packet](MAILING_PACKET.md)). Same attorney, same purpose → **byte-identical or virtually byte-identical** with the CBS media; not hash-verified.
